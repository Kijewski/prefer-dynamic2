...
fn main() -> Result<(), Error> {
        ...
        .map_err(|err| Error::ReadDir(err, lib_path.to_owned()))?
    {
        ...
        #[cfg(unix)]
        std::os::unix::fs::symlink(&src, &dest).map_err(|err| Error::Symlink(err, src, dest))?;
        #[cfg(windows)]
        junction::create(&dest, &src).map_err(|err| Error::Symlink(err, src, dest))?;
...
enum Error {
    ...
    Symlink(std::io::Error, PathBuf, PathBuf),
...
impl std::error::Error for Error {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            ...
            Error::RustcRun(err) | Error::ReadDir(err, _) | Error::Symlink(err, _, _) => Some(err),
        }
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Host => {
                write!(
                    f,
                    "Your host platform must be a `unix` (Linux, Mac, *BSD, ...) or `windows`.",
                )
            }
            Self::EnvVar(path) => {
                write!(
